...
function encodeAuthenticationResponse(assertion) {
  return {
    ...
    response: {
      ...
      userHandle:        assertion.response.userHandle ? b64uEncode(assertion.response.userHandle) : undefined,
    },
  };
}
...
async function boot() {
  try {
    ...
    loginHasPasskeys = !!status.hasPasskeys;
...
function showLoginScreen() {
  showScreen('login');
  if (loginHasPasskeys) {
    document.getElementById('login-passkey-section').classList.remove('hidden');
    document.getElementById('login-password-section').classList.add('hidden');
  } else {
    document.getElementById('login-passkey-section').classList.add('hidden');
    document.getElementById('login-password-section').classList.remove('hidden');
    document.getElementById('login-back-to-passkey-row').classList.add('hidden');
  }
}
...
async function setupRegisterPasskey() {
  hideErr('setup-error2');
  if (!window.PublicKeyCredential) {
    return showErr('setup-error2', 'This browser does not support passkeys');
  }
  const username = document.getElementById('setup-username').value.trim();
  const password = document.getElementById('setup-password').value;
  try {
    const opts = await api('POST', '/api/webauthn/register/start', { username, password }, false);
    if (opts.error) return showErr('setup-error2', opts.error);
    const cred = await navigator.credentials.create({ publicKey: decodeRegistrationOptions(opts) });
    const response = encodeRegistrationResponse(cred);
    const result = await api('POST', '/api/webauthn/register/finish', response, false);
    if (result.error) return showErr('setup-error2', result.error);
    loginHasPasskeys = true;
    showLoginScreen();
  } catch (e) {
    if (e.name === 'NotAllowedError') return showErr('setup-error2', 'Passkey registration was cancelled');
    showErr('setup-error2', 'Failed to register passkey');
    console.error(e);
  }
}
...
function showPasswordLogin() {
  document.getElementById('login-passkey-section').classList.add('hidden');
  document.getElementById('login-password-section').classList.remove('hidden');
  if (!loginHasPasskeys) document.getElementById('login-back-to-passkey-row').classList.add('hidden');
}
...
async function loginWithPasskey() {
  hideErr('login-error-pk');
  if (!window.PublicKeyCredential) {
    return showErr('login-error-pk', 'This browser does not support passkeys');
  }
  try {
    const opts = await api('POST', '/api/webauthn/login/start', {}, false);
    if (opts.error) return showErr('login-error-pk', opts.error);
    const assertion = await navigator.credentials.get({ publicKey: decodeAuthenticationOptions(opts) });
    ...
    if (result.error) return showErr('login-error-pk', result.error);
    accessToken = result.accessToken;
    await loadApp();
  } catch (e) {
    if (e.name === 'NotAllowedError') return showErr('login-error-pk', 'Passkey sign-in was cancelled');
    showErr('login-error-pk', 'Passkey sign-in failed');
    console.error(e);
  }
}
...
async function loadApp() {
  ...
  appGames   = data.games   || (typeof getInitialGames === 'function' ? getInitialGames() : {});
  appProfile = data.profile || (typeof getDefaultProfile === 'function' ? getDefaultProfile() : {});
...
function switchTab(tab) {
  ...
  if (tab === 'pending' && typeof renderPendingTab === 'function') renderPendingTab();
  if (tab === 'settings') {
    ...
    if (typeof loadRecoveryCodeCount === 'function') loadRecoveryCodeCount();
  }
}
...
function updateMeta() {
  const total   = Object.values(appGames).reduce((a,c) => a + (c ? c.length : 0), 0);
  const played  = (appGames.played||[]).length;
  const topMeta = document.getElementById('top-meta');
  if (topMeta) topMeta.textContent = `${total} games · ${played} played`;
  ...
  const qhm = document.getElementById('queue-hours-meta');
  if (qhm) qhm.textContent = `~${Math.round(qh)}h in queue`;
}
...
function renderGlobalResults() {
  ...
  const grl = document.getElementById('global-results-label');
  if (grl) grl.textContent = `${results.length} result${results.length===1?'':'s'} across all categories`;
  document.getElementById('game-legend').innerHTML = '';
...
async function moveGame(id, fromCat, toCat) {
  ...
  if (data.error) { if(typeof showToast==='function') showToast(`Error: ${data.error}`); return; }
  appGames[fromCat] = appGames[fromCat].filter(g => g.id !== id);
  appGames[toCat] = [...(appGames[toCat]||[]), data.game];
  updateMeta(); renderGames();
  if(typeof showToast==='function') showToast(`Moved “${game.title}” to ${CAT_LABELS[toCat]||toCat}`, () => restoreGames(before));
}
...
async function markPlayed(id, fromCat) {
  ...
  updateMeta(); renderGames();
  if(typeof showToast==='function') showToast(`Marked “${game.title}” as played`, () => restoreGames(before));
}
...
function setRank(id, cat, newRank) {
  ...
  if(typeof showToast==='function') showToast(`Moved “${moved.title}” to rank ${target}`, () => restoreGames(before));
}

function openAddGame() {
  if (typeof openGameModal !== 'function') return;
  openGameModal({
    title: 'Add Game',
    ...
    onSubmit: async (fields) => {
      ...
      if (data.error) { if(typeof showToast==='function') showToast(`Error: ${data.error}`); return false; }
      appGames.inbox = [...(appGames.inbox||[]), data.game];
      updateMeta(); renderGames();
      if(typeof showToast==='function') showToast(`Added “${data.game.title}” to Inbox`);
      return true;
    }
  });
}
...
function openEditGame(id, cat) {
  ...
  if (!game || typeof openGameModal !== 'function') return;
  openGameModal({
    title: 'Edit Game',
    ...
    onSubmit: async (fields) => {
      ...
      renderGames();
      if(typeof showToast==='function') showToast(`Saved changes to “${data.game.title}”`, () => restoreGames(before));
      return true;
    }
  });
}

// ── Complete Delete Game Logic (Fixed Truncation) ────────────
...
// PROFILE
// ═══════════════════════════════════════════════════════════
let profileEditingIdx = null; // index of section being edited, or null

function renderProfile() {
  const el = document.getElementById('profile-content');
  const sections = Array.isArray(appProfile) ? appProfile : [];
  document.getElementById('profile-btns').innerHTML = '';

  const cards = sections.map((s, i) => {
    if (profileEditingIdx === i) {
      return `<div class="profile-section-card">
        <div class="profile-section-edit">
          <input class="profile-name-input" id="pname-${i}" value="${esc(s.name)}" placeholder="SECTION NAME">
          <textarea class="profile-text-textarea" id="ptext-${i}">${esc(s.text)}</textarea>
          <div class="profile-section-edit-btns">
            <button class="btn btn-gold" style="font-size:12px;padding:5px 12px" onclick="saveSection(${i})">Save</button>
            <button class="btn btn-ghost" style="font-size:12px;padding:5px 12px" onclick="cancelEditSection()">Cancel</button>
            <button class="btn btn-ghost" style="font-size:12px;padding:5px 12px;color:var(--danger,#f87171);margin-left:auto" onclick="deleteSection(${i})">Delete</button>
          </div>
        </div>
      </div>`;
    }
    return `<div class="profile-section-card">
      <div class="profile-section-header">
        <div class="profile-section-name">${esc(s.name)}</div>
        <div class="profile-section-actions">
          <button class="btn btn-ghost" style="font-size:11px;padding:3px 10px" onclick="editSection(${i})">Edit</button>
        </div>
      </div>
      <div class="profile-body">${esc(s.text)}</div>
    </div>`;
  }).join('');

  const addBtn = profileEditingIdx === null
    ? `<div class="profile-add-section"><button class="btn btn-ghost" onclick="addSection()">+ Add Section</button></div>`
    : '';

  el.innerHTML = `<div class="profile-view">${cards}${addBtn}</div>`;
}

function editSection(i) { profileEditingIdx = i; renderProfile(); }
function cancelEditSection() { profileEditingIdx = null; renderProfile(); }

async function saveSection(i) {
  const name = document.getElementById(`pname-${i}`).value.trim();
  const text = document.getElementById(`ptext-${i}`).value;
  if (!name) return;
  const sections = Array.isArray(appProfile) ? [...appProfile] : [];
  sections[i] = { name, text };
  appProfile = sections;
  profileEditingIdx = null;
  await saveData();
  renderProfile();
}
...
function addSection() {
  const sections = Array.isArray(appProfile) ? [...appProfile] : [];
  sections.push({ name: 'NEW SECTION', text: '' });
  appProfile = sections;
  profileEditingIdx = sections.length - 1;
  renderProfile();
}
...
async function loadPasskeyList() {
  ...
  if (!list) return;
  const passkeys = await api('GET', '/api/webauthn/credentials');
  if (!passkeys || passkeys.error) { list.innerHTML = ''; return; }
  if (passkeys.length === 0) {
    list.innerHTML = '<div style="font-size:13px;color:var(--muted);">No passkeys registered.</div>';
    return;
  }
  list.innerHTML = passkeys.map(p => `
    <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:var(--surface);border:1px solid var(--border);border-radius:6px;margin-bottom:8px;">
      <div>
        <div style="font-size:13px;color:var(--text);">${escHtml(p.deviceName || 'Device')}</div>
        <div style="font-size:11px;color:var(--muted);">Added ${new Date(p.createdAt).toLocaleDateString()}</div>
      </div>
      <button class="btn btn-ghost btn-sm" onclick="removePasskey('${escHtml(p.credentialId)}')">Remove</button>
    </div>
  `).join('');
}
...
async function addPasskeyDevice() {
  ...
  msg.className = ''; msg.textContent = 'Starting registration…';
  if (!window.PublicKeyCredential) {
    msg.className = 'error-msg'; msg.textContent = 'This browser does not support passkeys'; return;
  }
  try {
    const opts = await api('POST', '/api/webauthn/add-device/start', {});
    if (opts.error) { msg.className = 'error-msg'; msg.textContent = opts.error; return; }
    const cred     = await navigator.credentials.create({ publicKey: decodeRegistrationOptions(opts) });
    const response = encodeRegistrationResponse(cred);
    const result   = await api('POST', '/api/webauthn/add-device/finish', response);
    if (result.error) { msg.className = 'error-msg'; msg.textContent = result.error; return; }
    msg.className = 'success-msg'; msg.textContent = 'Device registered!';
    loginHasPasskeys = true;
    await loadPasskeyList();
  } catch (e) {
    if (e.name === 'NotAllowedError') { msg.className = 'error-msg'; msg.textContent = 'Registration cancelled'; return; }
    msg.className = 'error-msg'; msg.textContent = 'Registration failed';
    console.error(e);
  }
}
